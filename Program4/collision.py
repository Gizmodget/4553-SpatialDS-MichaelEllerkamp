...
@date -  10/16/2015
@description - This program uses pantograph to paint our records as points on a page.
...
class Rock():
    ...
    def xInBounds(self,bounds,x):
        if x >= (bounds.maxX - (self.radius)) or x <= (bounds.minX + (self.radius)):
            return False

        return True

    def yInBounds(self,bounds,y):
        if y >= (bounds.maxY - (self.radius)) or y <= (bounds.minY + (self.radius)):
            return False
...
#                    print neih
                #swapping x vector, y vector, and velocity
                    ...
                    placeholdy = r.vectorOps.dy
                    placeholdvel = r.velocity
                    r.vectorOps.dx = neih.vectorOps.dx
                    r.vectorOps.dy = neih.vectorOps.dy
                    r.velocity = neih.velocity
                    neih.vectorOps.dx = placeholdx
                    neih.vectorOps.dy = placeholdy
                    neih.velocity = placeholdvel
                #safety valve as when the nodes grow they can trap each other.
                #this pushes one node till they are safe from being trapped by
                #their mutual growth
