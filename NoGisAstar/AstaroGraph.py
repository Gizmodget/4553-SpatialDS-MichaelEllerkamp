...
@date -  12/06/2015
@description - This program uses panto graph to visual an Astar route with added
features to make the pathing interesting. These include walls, special terrain cells,
and a slow path visualization 

@resources - Vast majority of the code was provided by Dr. Griffin
"""
...
class AStarGridNode(AStarNode):
    def __init__(self, x, y):
        ...
        self.iswater = False
    #if cell is a water cell then the cost is 5X greater than a normal cell
    def move_cost(self, other):
        diagonal = abs(self.x - other.x) == 1 and abs(self.y - other.y) == 1        
        if self.iswater == True: 
            return 70 if diagonal else 50              
        else:        
...
def make_graph(mapinfo):
    ...
    for x, y in product(range(mapinfo['width']), range(mapinfo['height'])):
        node = nodes[x][y]
        #if a cell is a water cell then a turn the boolean to true for the cost method
        if [x,y] in mapinfo['water']:
            node.iswater = True
        graph[node] = []
...
class DrawAstar(pantograph.PantographHandler):
    ...
    def setup(self):
        ...
        self.water = []                 #cells in the grid to become water
        self.adjWater = []              # adjusted for astar because each grid cell is "water" time big.
    ...
    def startAstar(self):
        self.pathtick = 1        
        # Initialize Astar
        graph, nodes = make_graph({"width": self.width/self.block, "height": self.height/self.block, "obstacle": self.adjObstacles, 'water':self.adjWater})
    ...
    StartAstar resets pathtick to 1 so that everytime we start astar
    the path will update slowly
    """
    def update(self):
...
#        self.drawPatch(400,400,900,900,'brick.png')
    ...
    """
    Someone's premade Patch function. Boring but could come in later handy.
    """
    ...
    """
    Draw the found path
    each time this is called by pantograph the length of the shown path
    increases by 1
    """
    def drawPath(self, pathtick):
        i = 0        
        for p in self.path:
            if i < pathtick:            
                # Blow up the cordinates pulled from Astar to fit our grid
                x,y = self.adjustCoords(p.x*self.block,p.y*self.block)
                self.fill_rect(x, y, self.block , self.block , "#FF0")
                i += 1
    ...
    an already "obstacled" cell, then it turns the cell into water,
    if clicked again then back to a normal cell
    """
    def toggleObstacle(self,x,y):
        ...
        elif [gridX,gridY] not in self.water:
            self.water.append([gridX,gridY])
            self.adjWater.append([adjX,adjY])
        elif [gridX,gridY] in self.water:    
            self.obstacles.remove([gridX,gridY])
            self.adjObstacles.remove([adjX,adjY])

            self.water.remove([gridX,gridY])
            self.adjWater.remove([adjX,adjY])
        else:    
            self.water.remove([gridX,gridY])
            self.adjWater.remove([adjX,adjY])
    ...
    """
    Draws the water, blue for water <3
    """
    def drawWater(self):
        for r in self.water:
            self.fill_rect(r[0], r[1], self.block , self.block , "#00F")
    """
    ...
    s removes paths
    w premade model
    q to turn the path from start to finish into a wall, then removes
      path, start, and end.
    """
    def on_key_down(self,e):
        ...
        if e.key_code==32: # S key
            pass
        if e.key_code==87: # w key
            self.refreshWorld()
            self.water = []
            self.obstacles = []
            self.adjWater = []
            self.adjObstacles = []            
            self.addFinish(1*self.block,5*self.block)
            self.addStart(10*self.block,5*self.block)
            self.toggleObstacle(1*self.block,4*self.block)
            self.toggleObstacle(2*self.block,5*self.block)
            self.toggleObstacle(2*self.block,5*self.block)
            self.toggleObstacle(2*self.block,6*self.block)
            self.toggleObstacle(2*self.block,6*self.block)
            self.toggleObstacle(2*self.block,7*self.block)
            self.toggleObstacle(2*self.block,7*self.block)
            self.startAstar()
        if e.key_code==81: # q key
            self.startAstar()            
            for p in self.path:
                # Blow up the cordinates pulled from Astar to fit our grid
                x,y = self.adjustCoords(p.x*self.block,p.y*self.block)
    ...
    """
    s erases the path, so you can go again
    """
